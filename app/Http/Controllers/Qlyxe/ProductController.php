...
class ProductController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $this->_data['items'] = DB::table('products as A')
            ...
            ->orderBy('A.id','desc')
            ->paginate(25);
        return view('qlyxe.products.index',$this->_data);
    }
    ...
    public function store(Request $request){
        ...
        if ($valid->fails()) {
            ...
            return redirect()->route('qlyxe.product.index',['type'=>$this->_data['type']])->with('success','Thêm dữ liệu <b>'.$product->languages[0]->title.'</b> thành công');
        }
    ...
    public function edit($id){
        ...
        if ($this->_data['item'] !== null) {
            ...
            return view('qlyxe.products.edit',$this->_data);
        }
        return redirect()->route('qlyxe.product.index',['type'=>$this->_data['type']])->with('danger', 'Dữ liệu không tồn tại');
    }
    ...
    public function delete($id){
        ...
        if ($product !== null) {
            ...
            if( $product->delete() ){
                return redirect()->route('qlyxe.product.index',['type'=>$this->_data['type']])->with('success', 'Xóa dữ liệu <b>'.$deleted.'</b> thành công');
            }else{
                return redirect()->route('qlyxe.product.index',['type'=>$this->_data['type']])->with('danger', 'Xóa dữ liệu bị lỗi');
            }
        }
        return redirect()->route('qlyxe.product.index',['type'=>$this->_data['type']])->with('danger', 'Dữ liệu không tồn tại');
    }

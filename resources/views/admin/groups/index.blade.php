	...
	<div class="col-md-12">
		<div class="portlet box green">
			<div class="portlet-title">
                ...
                <div class="actions">
                    <a href="{{ route('admin.group.create') }}" class="btn btn-sm btn-default"> Thêm mới </a>
                    <div class="btn-group">
					...
					<table class="table table-bordered table-condensed">
						...
						<tbody>
                            ...
                            <tr id="record-{{ $item->id }}">
                                ...
                                <td align="center"> <input type="text" name="priority" class="form-control input-mini input-priority" value="{{ $item->priority }}" data-ajax="act=update_priority|table=groups|id={{ $item->id }}|col=priority"> </td>
                                <td align="center"><a href="{{ route('admin.group.edit',['id'=>$item->id]) }}"> {{ $item->display_name }} </a></td>
                                <td align="center"> {{ $item->created_at }} </td>
                                <td align="center">
                                    <button class="btn btn-sm btn-status btn-status-publish btn-status-publish-{{ $item->id.' '.( ($item->status == 'publish') ? 'blue' : 'default' ) }}" data-loading-text="<i class='fa fa-spinner fa-pulse'></i>" data-ajax="act=update_status|table=groups|id={{ $item->id }}|col=status|val=publish"> Hiển thị </button>
                                </td>
                                <td align="center">
                                    <a href="{{ route('admin.group.edit',['id'=>$item->id]) }}" class="btn btn-sm blue" title="Chỉnh sửa"> <i class="fa fa-edit"></i> </a>
                                    <form action="{{ route('admin.group.delete',['id'=>$item->id]) }}" method="post">
                                        {{ csrf_field() }}
                                        {{ method_field('DELETE') }}
